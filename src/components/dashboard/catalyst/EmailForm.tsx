...
import { Send, Mail, MessageSquare, Users, Clock, Pause, Play, Square, CheckCircle2, XCircle, ImagePlus, Eye } from 'lucide-react';
import { EmailFormData, EmailJobState } from '@/App';
...
export const EmailForm: React.FC<EmailFormProps> = ({ 
  ...
  const handleApplyImage = (html: string) => {
    onFormDataChange({ ...formData, content: formData.content + '\n' + html });
  };
    ...
    <Card className="shadow-medium hover:shadow-large transition-all duration-300">
      <CardHeader>
        <div className="flex items-center space-x-2">
          ...
          <CardTitle className="text-lg">Bulk Email Sender</CardTitle>
        </div>
        <CardDescription>
          Send a customized email to multiple users from your verified Catalyst 'From' address.
        </CardDescription>
      </CardHeader>
          ...
          <div className="pt-4 border-t border-border">
              {!isProcessing ? (
                <Button
                    ...
                    variant="premium"
                    size="lg"
                    disabled={!formData.emails.trim() || !formData.subject.trim() || !formData.content.trim() || !fromEmail}
                    className="w-full"
                >
