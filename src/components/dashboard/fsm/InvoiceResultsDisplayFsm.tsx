...
import { CheckCircle2, XCircle, Eye, Hash, Mail, Clock, BarChart3, Download, Search, Hourglass, Paperclip } from 'lucide-react';
...
export const InvoiceResultsDisplayFsm: React.FC<ResultsDisplayProps> = ({ 
  results, 
  isProcessing, 
  isComplete,
  totalToProcess,
  ...
  const filteredResults = useMemo(() => {
    ...
    return results.filter(r => 
      r.workOrderId.toLowerCase().includes(filterText.toLowerCase()) ||
  ...
  const successCount = results.filter(r => r.success && r.step === 'send').length;
  const errorCount = results.filter(r => !r.success).length;
  const progressPercent = totalToProcess > 0 ? (results.length / totalToProcess) * 100 : 0;

  const handleExport = () => {
    const header = "Work Order ID,Email,Status,Details\n";
    const csvContent = filteredResults.map(r => {
      const status = r.success && r.step === 'send' ? 'Success' : 'Failed';
      const details = (r.details || r.error || '').replace(/"/g, '""');
      return `${r.workOrderId},${r.email},${status},"${details}"`;
    }).join('\n');
    ...
    link.setAttribute("download", "fsm-invoice-results.csv");
  ...
  return (
    <Card className="shadow-medium hover:shadow-large transition-all duration-300">
      <CardHeader className="pb-4">
        ...
        <CardDescription>
          {isProcessing ? 'Creating and sending invoices in real-time...' : 
           isComplete ? `All ${totalToProcess} invoices have been processed.` : 
           'View results below.'}
        </CardDescription>
      </CardHeader>
                  ...
                  {filteredResults.slice().reverse().map((result, index) => (
                    <tr 
                      ...
                      className={`transition-colors hover:bg-muted/30 ${
                        result.success && result.step === 'send' ? 'bg-success/5' : !result.success ? 'bg-destructive/5' : ''
                      }`}
                    >
                      ...
                      <td className="px-4 py-3">
                        {!result.success ? (
                          ...
                          <Badge variant="outline" className="text-muted-foreground">
                            <Hourglass className="h-3 w-3 mr-1 animate-spin" />
                            Created
                          </Badge>
                        ) : (
                          <Badge variant="success" className="bg-success/10 text-success">
                      ...
                      <td className="px-4 py-3 text-center">
                        <Dialog>
                          ...
                          <DialogContent className="max-w-3xl bg-card border-border shadow-large">
                            <DialogHeader>
                              <DialogTitle>
                                Full Response - {result.workOrderId}
                              </DialogTitle>
                            </DialogHeader>
