...
import { Profile, FsmInvoiceJobs, FsmInvoiceJobState, FsmInvoiceFormData } from '@/App';
import { InvoiceFormFsm } from './InvoiceFormFsm';
import { InvoiceResultsDisplayFsm } from './InvoiceResultsDisplayFsm';
...
interface FsmInvoiceDashboardProps {
  jobs: FsmInvoiceJobs;
...
export const FsmInvoiceDashboard: React.FC<FsmInvoiceDashboardProps> = ({
    jobs,
    setJobs,
    socket,
  ...
  const handleFormDataChange = (newFormData: FsmInvoiceFormData) => {
  ...
  const handleFormSubmit = () => {
    ...
    const invoiceData = currentFormData.invoiceData.split('\n').map(line => {
        const [workOrderId, email] = line.split(',').map(item => item.trim());
        return { workOrderId, email };
    }).filter(item => item.workOrderId && item.email);

    if (invoiceData.length === 0) {
        toast({ title: "No valid invoice data provided", variant: "destructive" });
        return;
    }
    ...
    setJobs(prev => ({
        ...
        [activeProfileName]: {
            ...
            processingTime: 0,
            totalToProcess: invoiceData.length,
            currentDelay: currentFormData.delay,
            filterText: '',
        }
    }));
    socket.emit('startBulkCreateAndSendFsmInvoice', {
        invoiceData,
        delay: currentFormData.delay,
  ...
  const handlePauseResume = () => {
    ...
    socket.emit(isPaused ? 'resumeJob' : 'pauseJob', { profileName: activeProfileName, jobType: 'fsm-invoice' });
  ...
  const handleEndJob = () => {
    if (!socket || !activeProfileName) return;
    socket.emit('endJob', { profileName: activeProfileName, jobType: 'fsm-invoice' });
  };
